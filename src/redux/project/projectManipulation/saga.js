...
const postProject = function*() {
  yield takeEvery(actions.POST_PROJECT, function*({ newProject }) {
    ...
    const postProjectResponse = yield call(() =>
      ProjectApi.postProject(newProject)
    );
...
const modifyProject = function*() {
  yield takeEvery(actions.MODIFY_PROJECT, function*({
    projectId,
    targetProject
  }) {
    yield put({
    ...
    const modifyProjectResponse = yield call(() =>
      ProjectApi.modifyProject(targetProject)
    );
...
export default function*() {
  yield all([fork(postProject), fork(modifyProject), fork(deleteProject)]);
}
