...
export const getProjects = (page, limit) =>
  fetchHelper
    .get(`/projectMain?page=${page}&limit=${limit}`)
    .then(response => checkExpirity(response.token));

export const postProject = newProject =>
  fetchHelper
    .post("/projectMain", newProject)
    .then(response => checkExpirity(response.token));

export const modifyProject = (projectId, replaceProject) =>
  fetchHelper
    .put(`/projectMain/${projectId}`, replaceProject)
    .then(response => checkExpirity(response.token));

export const deleteProject = projectId =>
  fetchHelper
    .delete(`/projectMain/${projectId.toString()}`, projectId)
    .then(response => checkExpirity(response.token));
